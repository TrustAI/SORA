...
class DirectBase(ABC):
    ...
    def solve(self):
        ...
        while self.nb_iter <= self.max_iter and not self.runout_query and not self.reach_max_deep:
            ...
            self.logger.debug(f'[-] {self.nb_iter} th iter: Global minimum: {self.rcd.minimum:.6f} (estimated lower bound: {self.local_low_bound:.6f}). Number of funcation evaluation: {self.rcd.last_center + 1}, found largest slope: {np.max(self.rcd.local_slope)}')
